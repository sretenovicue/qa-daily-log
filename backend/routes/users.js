...
router.get('/', wrap(async (req, res) => {
  const { rows } = await pool.query(
    'SELECT id, email, username, role, active, created_at FROM users ORDER BY created_at ASC'
  );
  res.json(rows);
}));
...
router.post('/', wrap(async (req, res) => {
  ...
  const result = await pool.query(
    'INSERT INTO users (email, username, password_hash, role) VALUES ($1, $2, $3, $4) RETURNING id, email, username, role, active, created_at',
