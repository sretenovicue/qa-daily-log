...
test.describe('Auth — login forma', () => {
  ...
  test('pogrešni kredencijali pokazuju grešku', async ({ page }) => {
    await page.getByRole('textbox', { name: 'Email ili korisničko ime' }).fill('nepostoji@test.com');
  ...
  test('prelaz na registraciju prikazuje nova polja', async ({ page }) => {
    ...
    await expect(page.getByRole('textbox', { name: 'Korisničko ime' })).toBeVisible();
